...
openshift admin ca create-server-cert --key=hawkular-metrics.key --cert=hawkular-metrics.crt --hostnames=hawkular-metrics.example.com --signer-cert=/etc/origin/master/ca.crt --signer-key=/etc/origin/master/ca.key --signer-serial=/etc/origin/master/ca.serial.txt
