...
POD=$(oc process logging-deployer-template -n openshift -v KIBANA_HOSTNAME=kibana.example.com,ES_CLUSTER_SIZE=1,PUBLIC_MASTER_URL=https://ose3-master.example.com:8443 | oc create -f - | cut -d '"' -f2)
